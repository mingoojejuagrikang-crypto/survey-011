...
- **[CLIP-BG-MUTE-1] → 판정 보류(여전히 n=1).** 이번 hidden 창(7,058초)은 **앱 이탈**이라
  ...
  시나리오가 필요하다.
- **[SCREEN-LOCK-1] 계측 → 🔴 0건 미발화.** 화면 끄기 축 판정 불가.
...
- **계측 공백:** 모달↔마이크 트랙의 **인과를 말하는 이벤트가 없다**(회차 SSOT §5-G).
  5/10 시간 인접으로 추론했다.
- 🔑 **다음 조사 축:** **왜 5건은 죽고 5건은 멀쩡한가.** 모달 길이로는 안 갈린다 —
  무사한 09:30:46은 145초(최장급), 죽은 11:47:18은 65초다. 구분 요인 미확정.
  후보: 모달 진입 시점의 녹음 활성 여부 · 직전 입력 장치 · STT 재시작 타이밍.
- **현재 상태:** 🔴 **OPEN** — 우선순위 2번.
