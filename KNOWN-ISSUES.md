...
- **해결·회피(v0.20.0):** ① 인식 허용범위를 사용자 다이얼로 노출(`recognitionTolerance`, 기본 0.60, 입력탭). ② **계측 우선**: `speech.ts`가 final마다 `raw_confidence:<n>|absent` 로깅해 "0.0 반환" vs "부재(→1 강제)"를 구분. 다음 세션에서 iOS가 실제로 confidence를 채우는지 확정 후 `?? 1` 동작 수정 판단(**추측 기반 변경 금지**).
- **출처:** `2026-06-24 v0.19.0 실기기 분석` → **survey-011 v0.20.0**
- **현재 상태:** ⚠️주시(계측만) — 진짜 방어선은 TTS 되읽기. 고신뢰 침묵 오커밋 능동확인(BL-1)은 confidence 실측 후 설계.
...
- **현재 상태:** ⚠️주시(계측만) — 다음 세션 로그에서 `clip_trim_failed` 빈도·원인 확인 후 실제 트림 폴백 개선 판단. 나머지 3행(트림 정상)은 **루프백 하니스 청취 대기(BL-6)**.
