...
## §14. 08-14 R1 이후 추기 — 신규 검출 1건 판정

### 14-A. `BeepPicker.tsx`(+`previewBeep`) — 미사용 파일 검출 · 🔴 삭제 금지(보존 계약)

R1 착수 재실측(08-14)에서 knip 미사용 파일 검출이 판정표 기준 2건 대비 **+1건**: `BeepPicker.tsx`.
경위 실측 — v0.46.0 WP-I(`e8d825d`)가 렌더를 제거했고(`SessionOptionsSection.tsx:8-9` import 주석
처리) **민구 지시로 파일은 의도 보존**됐다. 즉 §5(상속 유물)가 아니라 **의도된 드리프트**다.
→ **처분: 존치.** 이후 knip 대조에서 미사용 파일 1건(BeepPicker)은 기대 기준선이다.
재도입 또는 삭제 결정은 민구 몫.
