...
- **원인(4b0185c 코드 추적, 3중):** ① modify 핸들러의 `if (awaiting.isModify) { pendingCmd?.saveDefault(); await say(…); return; }` 분기가 `announceField()`와 달리 **클립을 재시작하지 않아** 다음 발화가 결정적으로 미녹음(`cancel` 분기도 동일 구조 — 잠재 동일 결함). ② `clip_empty`의 unlink가 in-flight `persistSession`에 되덮임([CLIP-POINTER-1] 레이스 — 이동 당시 표기 [CLIP-3]). ③ 재녹음 커밋이 캐노니컬 키 `sess:row:colId`를 재사용해 빈 캡처 시 **이전 값 음성이 그 키 밑에 그대로 재생 대상**으로 남음.
