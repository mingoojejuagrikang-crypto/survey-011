...
/* 🔴 v0.46.0 콜드 리뷰 L2-1 처방 — 아래 두 픽스처는 **값을 반복시킨다**(정당 파손).
 *    승격 기준이 「고유값 2~20개」에서 **평균 반복 ≥ 2회**(`sheets.ts: OPTIONS_MIN_REPEAT`)로
 *    바뀌었다. 종전 픽스처는 값이 한 번씩만 나와 평균 1.0이었고, 그 성질이 곧 **자유서술 칸**의
 *    성질이다 — 그래서 이제 승격되지 않는다.
 *    🔑 **이 스펙들이 재는 계약(21개 이상도 리스트 · 최근순 정렬 · 기본 선택 = 최근 값)은 그대로다.**
 *    바꾼 것은 표본이 그 계약에 도달하는 조건뿐이다. */

test('J-2 — 고유값 21개 이상도 리스트가 된다(종전 ≤20 상한 폐지)', () => {
  // 종전 규칙에서는 21개째부터 'text'로 떨어져 리스트가 통째로 사라졌다.
  // 25개 농가가 각각 2회 등장(평균 반복 2.0) — 실제 시트에서 농가명이 갖는 성질이다.
  const farms = Array.from({ length: 25 }, (_, i) => `농가${i}`);
  const sample = [...farms, ...farms].map((f) => [f]);
...
test('J-2 — 승격된 선택지도 최근 등장 우선이고, 기본 선택은 가장 최근 값이다', () => {
  // 세 농가가 두 바퀴 등장 — 최근순 정렬 계약은 「마지막 등장이 먼저」이므로 결과는 불변이다.
  const [col] = inferColumns(
    ['농가명'],
    [['이원창'], ['강남호'], ['위미리3407'], ['이원창'], ['강남호'], ['위미리3407']],
  );
