...
test('B4 — 마지막 행 완료 후 자동 종료 안 함(대기) · 값 발화는 재안내 · "종료"로만 종료 + 세션메타 tolerance 박제', async ({ page }) => {
  ...
  // ①-b hero가 정적 대기 라벨("N행 완료 — 명령 대기") — 마지막 컬럼을 다시 묻는 것처럼 보이지
  //    않아야 한다. (v0.34.0 A4 — '입력 완료'/'입력됨' 상태 제거, listening 전용화의 예외 1분기.)
  await expect(page.locator('text=명령 대기').first()).toBeVisible({ timeout: 2000 });
  await expect(page.locator('text=2행 완료').first()).toBeVisible({ timeout: 2000 });
...
test('B4 — 일시정지 패널의 종료 버튼으로도 마지막 행 대기 상태에서 종료된다(음성 외 경로)', async ({ page }) => {
  await setupAndStart(page);
  ...
  // 활성 하단에는 종료가 없으므로 일시정지 패널을 열어 종료 버튼을 탭한다.
  await page.locator('button[title="일시정지"]').click({ force: true });
