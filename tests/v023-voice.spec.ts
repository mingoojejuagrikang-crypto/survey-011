...
type LogEv = { type?: string; extra?: string; ttsText?: string; confidence?: number; meta?: { recognitionTolerance?: number } };
...
test('B1 — 이상치 카드가 중앙 흡수영역 안에 렌더 + 375px 긴이름/음수소수(-355.5) 잘림 0', async ({ page }) => {
  ...
  console.log('✓ 이상치 카드 표시 + 긴 항목명/음수소수 포함');
  ...
  const yAfter = (await endBtn.boundingBox())!.y;
...
test('B2 — 재질문 사유 큐: 머지 전(reaskReason 미존재)엔 안전하게 미표시, listening hero는 정상', async ({ page }) => {
  ...
  // 머지 후 검증 훅(Mack의 setReaskReason): reaskReason='low_confidence'→"신뢰도 낮음 — 다시 또렷이",
  //   'parse_failed'→"숫자 인식 실패 — 다시 또렷이". 통합 시 이 testid(reask-cue)+data-reason으로 확인.
  ...
  if (injected) {
    ...
    await expect(page.locator('[data-testid="reask-cue"]')).toContainText('숫자 인식 실패');
...
test('B3 — 마이크 재연결 버튼 탭 → 쿨다운 동안 "재연결 중…"·비활성 → ~3s 후 재활성', async ({ page }) => {
  ...
  // 평상시(쿨다운 아님): 활성 + "마이크 재연결".
  await expect(btn).toBeEnabled();
  await expect(btn).toContainText('마이크 재연결');
  ...
  // 쿨다운(3s) 경과 → 재활성("마이크 재연결" 복귀). 배너가 여전히 떠 있다는 전제(재연결 실패 케이스).
...
test('B4 — 마지막 행 완료 후 자동 종료 안 함(대기) · 값 발화는 재안내 · "종료"로만 종료 + 세션메타 tolerance 박제', async ({ page }) => {
  ...
  // ①-b hero가 "행 입력 완료"(complete) 상태 — 마지막 컬럼을 다시 묻는 것처럼 보이지 않아야 한다.
  await expect(page.locator('text=행 입력 완료').first()).toBeVisible({ timeout: 2000 });
...
test('B4 — 종료 "버튼"으로도 마지막 행 대기 상태에서 종료된다(음성 외 경로)', async ({ page }) => {
  await setupAndStart(page);
  ...
  // 하단 컨트롤바 "종료" 버튼(title="입력 종료")이 보이고, 탭하면 세션이 끝나 ready로 복귀한다.
  const endBtn = page.locator('button[title="입력 종료"]');
...
test('B2 — 저신뢰도(conf<허용범위) 발화 → 사유 큐 low_confidence + stt_rejected_low_confidence(tolerance 동봉), 성공 시 해제', async ({ page }) => {
  ...
  expect(lowConf?.extra).toBe('tolerance:0.6'); // 설정값 vs 신뢰도 대조 근거
