...
// 🔴 **최소 지원 규격 = iPhone 13 mini (375×812)** — 민구 확정 2026-08-04.
//    종전 격자의 `320`을 뺐다. 320은 지원 대상이 아니고, §C1에서 가장 어려운 케이스라
//    없는 제약에 설계를 맞추게 만든다.
//    ⚠️ **`v043-fit-group`의 320은 남긴다** — 그건 지원 범위가 아니라
//      *"폭이 320→402로 늘면 글자가 커지는가"* 를 재는 **fit 기제 압력 테스트**다. 성격이 다르다.
//    ⏭ **최대는 아직 1920이다.** 민구가 32인치(2560~3840)를 지원 범위로 넣겠다고 했으나
//      *"넓은 쪽은 더 많은 정보를 보여주면 된다"* 는 **별도 설계 축**이라 그 플랜에서 다룬다 —
//      지금 격자만 넓히면 `compareLabel/Value`(상한 없는 `vw`)가 356px·496px로 폭발한다.
// 🔴 v0.46.1(민구 실기기 08-07) — **402와 513을 넣었다.**
//    제보 9건 중 **7건이 402×513**이었는데(iPhone 402 폭 · Safari 크롬이 874−513=361px를 먹는다)
//    종전 격자에는 **폭 402가 아예 없었고** 높이 513도 없었다 — 즉 이 게이트는 민구가 실제로
//    쓰는 화면을 **구조적으로 한 번도 지나가지 않았다.** 72조합 → 81조합.
//    ⚠️ 늘린 만큼 느려진다. 줄이려면 402·513이 아니라 **덜 쓰이는 데스크톱 폭**을 빼라.
const WIDTHS = [375, 402, 430, 540, 768, 1024, 1280, 1600, 1920];
const HEIGHTS = [400, 500, 513, 568, 667, 800, 874, 1024, 1200];
