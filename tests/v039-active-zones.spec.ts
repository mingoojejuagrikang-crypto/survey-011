...
const HEADERS = ['조사일자', '농가명', '조사나무', ...VOICE_COLS.map((c) => c.name)];
const SHEET_ROWS = [
...
async function boot(page: Page, viewport = PHONE_402) {
...
async function zoneMetrics(page: Page) {
...
// ─── §[2] anomaly ───────────────────────────────────────────────────────────
async function triggerAnomaly(page: Page) {
...
async function fillAllRows(page: Page) {
