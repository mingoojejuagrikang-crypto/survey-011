 ...
 *  중앙 50은 **그대로다** — 위 단락의 회귀 판별식은 계속 유효하다.
 *  이 값을 다시 20/50/30으로 되돌리려면 F20 확정을 뒤집는 근거가 먼저 있어야 한다. */
...
// **컨트롤바가 따라 움직이고 중앙 흡수영역이 같이 늘었다 줄었다** 했다. 새 구조는 밴드를 하단
// 트랙(UI-b 기준 30%) **안**에 넣어 그 전달 경로를 끊는다.
