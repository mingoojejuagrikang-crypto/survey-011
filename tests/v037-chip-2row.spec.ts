 ...
 *
 *  왜 「공간을 회수해서 되돌린다」(아래 §방어① 주석)로 못 푸는가 — 세 제약이 동시에 닫는다:
 *   ① **회수할 빈 공간이 없다.** `lineHeight: normal`의 27% 회수는 v0.43.0 UI-b가 이미 했다
 *      (`ColumnChip.tsx:126` `lineHeight: 1`). 남은 여백은 padding 1cqh + border뿐이다.
 *   ② **위계 역전으로 막힌다.** 390을 살리려 `CHIP_TYPE.name`을 47.3cqh로 올리면 640에서
 *      name(75.3) > value(64.3)가 되어 *"항목명은 값 이하"* 계약(`heroLayout.ts` CHIP_TYPE 주석)을
 *      깬다. 390만 따로 키우는 수단이 없다 — 같은 공식이 모든 뷰포트를 덮는다.
 ...
 *  ⚠️ 390의 20.74px는 민구 실기기 확인 대상이다 — 산출물에 트레이드오프를 숫자로 남겼다. */
const CHIP_LABEL_BASELINE_PX = { standard: 37.38, short: 20.74 } as const;
