...
test('[E2E-19] 전체 플로우 — 설정→생성→시작→일시정지→재개→종료', async ({ page }) => {
  ...
  if (pauseAvail) {
    await pauseBtn.click({ force: true });
    await page.waitForTimeout(500);
    // v0.21.0 입력탭#1 — 화면 본문 TTS 에코(sess.lastTts) 렌더가 제거되어 "…재개됩니다" 안내
    //   문구가 더 이상 innerText에 뜨지 않는다. 일시정지 상태는 PausedCard 중앙 대형 카드로만
    //   표시된다. "일시정지"·"재시작"은 하단 명령어 칩(PRIMARY_COMMANDS)에도 상시 떠 있어 body
    //   substring으로는 일시정지 진입을 판별할 수 없으므로(vacuous), PausedCard 고유 안내문구로
    //   검증한다(음성 안내 say/setLastTts는 유지).
    const pausedCardShown = await page.locator('text=이라고 말하면 이어서 진행').first()
      .isVisible({ timeout: 3000 }).catch(() => false);
    expect(pausedCardShown).toBe(true);
