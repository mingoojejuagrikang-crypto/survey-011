...
interface LogEvt { type?: string; extra?: string; row?: number; colId?: string; clipKey?: string }
...
test('②③ 수정 재녹음 빈 캡처 → 포인터가 cmd 클립으로 재연결되고 이후 persist에서 생존한다', async ({ page }) => {
  ...
  // "수정 155.5" — 이 발화가 (1,c8):cmd1으로 저장된다(새 값 발화를 담은 cmd 클립).
  await fireStt(page, '수정 155.5', 600);
  ...
  expect(row1.audioClips?.c8, '재연결 포인터가 persist에 덮임(레이스)').toBe(`${sess.id}:1:c8:cmd1`);
});

test('②③ cmd 클립이 없는 빈 캡처 → unlink, 이후 persist에 깨진 포인터가 부활하지 않는다', async ({ page }) => {
  await setupAndStart(page);

  await waitForActiveChip(page, '횡경');
  await fireStt(page, '35.1', 600);

  // "수정"만(인라인 값 없음) → cascade — cmd 클립은 (1,c9) 키로 저장되므로 c8용 cmd는 없다.
  await waitForActiveChip(page, '종경');
  await fireStt(page, '수정', 600);
  await waitForActiveChip(page, '횡경');
