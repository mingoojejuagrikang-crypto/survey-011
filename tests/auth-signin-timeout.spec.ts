...
test('타임아웃(120s) 발화 후 지각 성공 콜백 — 리마운트 없이 자동으로 googleConnected 반영', async ({ page }) => {
  ...
  // v0.33.0 항목5 — 연결 3상태 카드(ConnectionStatusCard)도 같은 이메일을 표시하므로
  // (로그인됨 · email) 이메일 로케이터는 버튼으로 스코프해 strict mode 충돌을 피한다(의도 동일).
  await expect(page.locator('text=연결됨')).toBeVisible({ timeout: 5_000 });
  await expect(page.locator('button:has-text("slow2fa@example.com")')).toBeVisible();
});
...
test('정상 범위 내 로그인(타임아웃 전 도착)은 회귀 없이 그대로 성공', async ({ page }) => {
  ...
  await expect(page.locator('text=연결됨')).toBeVisible({ timeout: 5_000 });
  // v0.33.0 항목5 — 3상태 카드도 이메일을 표시하므로 버튼 스코프(위 테스트와 동일 이유).
  await expect(page.locator('button:has-text("slow2fa@example.com")')).toBeVisible();
