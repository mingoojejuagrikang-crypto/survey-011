...
test('입력탭#1 — 선택 칩에 순번 숫자 뱃지 + 불일치 라이브 미리보기 제거', async ({ page }) => {
  ...
  await expect(card.locator('[data-testid="opt-preview-c4"]')).toHaveCount(0);
  await expect(card.getByText('자동 입력:', { exact: false })).toHaveCount(0);
  console.log('✓ 불일치 라이브 미리보기 제거');
});
...
test('입력탭#1 — 중간값 해제 시 뒤 번호 당김, 재선택 시 맨 끝 번호로', async ({ page }) => {
  ...
  await expect(card.locator('[data-testid="opt-preview-c4"]')).toHaveCount(0);
  console.log('✓ 해제 → 뒤 번호 당김(C 3→2), 미리보기 없음');
  ...
  await expect(card.locator('[data-testid="opt-preview-c4"]')).toHaveCount(0);
  console.log('✓ 재선택 → 맨 끝 번호(B=3), 미리보기 없음');
});
