...
test.describe('v0.49 fix49b — 엔진 cancel 지점의 드레인 계약', () => {
  ...
  test('③ 큐 대기 발화의 시작 워치독이 앞 발화 재생 중 만료되지 않는다 (#5)', async () => {
    ...
    // ⚠️ **이 케이스가 재는 범위를 정확히 적어 둔다** — 「앞 발화가 **재생되는 동안**」이다.
    //   앞 발화가 **정상 종료한 뒤**의 창은 여기서 재지 않으며, 실측 결과 그 창은 **아직 열려
    //   있다**: 뮤트를 거는 쪽은 `engine.speak(u)` 직전(= enqueue 시점)이라 큐잉된 발화도 그때
    //   이미 걸지만, 푸는 쪽은 발화마다의 `done()`이고 `ttsMuted`는 refcount가 아닌 boolean이다.
    //   그래서 앞 발화의 `onend` 하나가 **뒤 발화 몫의 뮤트까지** 푼다(`onstart`에 재-뮤트가
    //   없다 — v0.4.x가 뮤트를 onstart에서 speak 직전으로 옮긴 계약이 그 자리를 비워 뒀다).
    //   그 잔여는 `#5`의 앵커 축이 아니라 **뮤트 소유권 모델**의 문제라 별건으로 기록했다:
    //   `[TTS-MUTE-REFCOUNT-1]`. 여기서 `.toBe(false)`로 굳히면 결함이 계약이 된다 — 그래서
    //   단언하지 않고 범위만 밝힌다.
  });
