...
export const CASES: StateCase[] = [
  ...
  },
  {
    ...
    // 🔴 **이 카드는 현재 화소 대조가 red다(1.08%, 임계 1%). 구조적 원인이 있다 — 인계 사항이다.**
    //  종전 corrected 카드의 수명은 `await speak(echo)`라 `widenTtsWindow`로 **테스트가 늘릴 수
    //  있었다**. 확정 플래시의 수명은 `CONFIRM_MS = 1500` 코드 상수라 **못 늘린다.** 프리뷰와
    //  실화면을 각각 캡처하는 이 하네스는 두 캡처가 같은 1.5초 창 안에 들어가야 하는데 그 여유가
    //  없다. 실측한 조합(전부 red):
    //    · `widenTtsWindow(3000)` 그대로            → **1.08%** (가장 가깝다 — 현재 상태)
    //    · `widenTtsWindow` 제거(동기 onend)        → 2.1%
    //    · `widenTtsWindow(3000)` + 정착 대기 250ms → 1.38% + `drift=4`(구조까지 어긋난다)
    //  `holdSelector`는 통과한다(`drift=0`·`unstable=0`) — 플래시를 잡는 것 자체는 된다.
    //  갈리는 것은 **두 캡처의 순간**이다. 근본 처방은 이 하네스가 과도 상태를 다루는 방식에
    //  있지 이 케이스 안에 없다(범위 밖으로 남긴다).
