...
test('B8 — micLost(트랙 사망 → clip_empty 래치) 시 글로우 red + 재연결 배너', async ({ page }) => {
  ...
  await expect(glow).toHaveAttribute('data-tone', 'red');
  await expect(statusControl).toHaveAttribute('data-tone', 'red');
  await expect(statusControl).toHaveAttribute('data-status', 'alert');
  const symbolAnimation = await statusControl.locator('[data-status-symbol]').evaluate(
    (el) => getComputedStyle(el).animationName,
  );
  expect(symbolAnimation).toBe('voice-status-fade');
  console.log('✓ micLost → 글로우+하단 상태심볼 red(이상치 없이도) + 마이크 재연결 배너');
});
