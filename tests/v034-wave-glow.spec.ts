...
test('B7 — 대기 카드: 항목명 + 70% 폭 세로 막대 파형, "듣는 중" 텍스트 제거', async ({ page }) => {
  ...
  expect(geometry.barHeight).toBe(78);
  expect(geometry.barCount).toBe(13);
  console.log('✓ 대기: 항목명 + 70% 폭 세로 막대 파형, "듣는 중" 텍스트 제거');
});

// v0.38.0 #10 — rAF가 공급한 가장 높은 --level을 읽는다. inset-wave의 시점별 transform과 무관하게
//   오디오 레벨 구독 결과를 결정적으로 검증한다.
async function waveMaxLevel(page: Page): Promise<number> {
  return page.locator('[data-testid="voice-waveform"]').evaluate((el) => {
    const bars = Array.from(el.querySelectorAll('span')) as HTMLElement[];
    let max = 0;
    for (const b of bars) {
      const level = parseFloat(b.style.getPropertyValue('--level'));
...
test('B7 — 파형 막대 실제 렌더 + 주입 레벨에 진폭 반응(FB-D)', async ({ page }) => {
  ...
  await page.waitForTimeout(200);
  const lo = await waveMaxLevel(page);
  expect(lo, '레벨 0 기본 세로 막대 높이').toBeCloseTo(0.35, 2);
  const idleBarHeight = await page.locator('[data-testid="voice-waveform"] span').first().evaluate(
    (el) => el.getBoundingClientRect().height,
  );
  expect(idleBarHeight, '무입력에도 얇은 선이 아닌 세로 막대로 보인다').toBeGreaterThan(20);
  const idleAnimation = await page.locator('[data-testid="voice-waveform"] span').nth(1).evaluate((el) => {
    const cs = getComputedStyle(el);
    return { name: cs.animationName, duration: cs.animationDuration };
  });
  expect(idleAnimation.name === 'none' || idleAnimation.name === '').toBeTruthy();
  expect(idleAnimation.duration).toBe('0s');

  // 레벨 0.85: 막대가 커져 최고 막대 높이가 유의미하게 늘어난다(레벨 반응).
  await injectLevel(page, 0.85);
  await page.waitForTimeout(250);
  const hi = await waveMaxLevel(page);
  expect(hi, '레벨↑ → 막대 높이↑').toBeGreaterThan(lo + 0.1);
  await expect.poll(
    () => page.locator('[data-testid="voice-waveform"] span').nth(1).evaluate((el) => {
      const cs = getComputedStyle(el);
      return { name: cs.animationName, duration: cs.animationDuration, delay: cs.animationDelay };
    }),
    { message: '레벨 유입 시 reference inset-wave cadence 적용' },
  ).toEqual({ name: 'inset-wave', duration: '0.94s', delay: '-0.073s' });
  console.log(`waveMaxLevel: level0=${lo} level0.85=${hi}`);
});
