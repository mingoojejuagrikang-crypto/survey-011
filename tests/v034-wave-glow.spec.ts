...
/** canvas stroke 호출 카운터 설치(R3-FIX-4와 동일 패턴) — 파형 draw 1회당 최소 1회 stroke. */
async function installStrokeCounter(page: Page) {
  await page.evaluate(() => {
    const proto = CanvasRenderingContext2D.prototype;
    const orig = proto.stroke;
    (window as unknown as { __strokes: number }).__strokes = 0;
...
const strokeCount = (page: Page) =>
  page.evaluate(() => (window as unknown as { __strokes: number }).__strokes);
...
test('R1-3 — complete 검토 대기: 글로우 레벨 rAF·파형 draw가 계속 살아 있다', async ({ page }) => {
  ...
  // 파형 draw 지속(정적 평선이 아니라 실시간).
  await installStrokeCounter(page);
  await injectLevel(page, 0.8);
  await page.waitForTimeout(500);
  const drawn = await strokeCount(page);
  expect(drawn, 'complete에서 파형 draw 지속').toBeGreaterThan(0);
  console.log(`✓ R1-3: complete에서 glowVar 갱신 + 파형 strokes=${drawn}`);
});

// R1-4 (Flash 테스트 노트, D13) — paused에서 파형 rAF가 실제로 멈춘다(수명주기 단언 — toBeVisible
//   같은 표면 단언이 아니라 draw 호출 정지/재개를 직접 계수).
test('R1-4 — 일시정지: 파형 rAF 실중지(스트로크 증가 0), 재시작 시 재개', async ({ page }) => {
  await boot(page);
  await startSession(page);
  await installStrokeCounter(page);
  await injectLevel(page, 0.8);
  await page.waitForTimeout(400);
  expect(await strokeCount(page), '활성 중엔 draw가 돈다').toBeGreaterThan(0);

  await page.locator('button[title="일시정지"]').click();
  await expect(page.locator('[data-testid="paused-card"]')).toBeVisible();
  await page.waitForTimeout(500); // effect 재실행(정적 평선 1회 draw) 정착
  const pausedStart = await strokeCount(page);
  await page.waitForTimeout(700); // 루프가 살아 있다면 ~21프레임(30fps)이 더 찍힌다.
  const pausedEnd = await strokeCount(page);
  expect(pausedEnd - pausedStart, 'paused 중 파형 draw 증가 0').toBe(0);

  await page.locator('button[title="재시작"]').click();
  await page.waitForTimeout(500);
  const resumed = await strokeCount(page);
  expect(resumed, '재시작 후 draw 재개').toBeGreaterThan(pausedEnd);
  console.log(`✓ R1-4: paused(+0) → resumed(+${resumed - pausedEnd})`);
});
