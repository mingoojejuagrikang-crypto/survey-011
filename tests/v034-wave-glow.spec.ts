...
test('B7 — 대기 카드: 항목명 + 70% 폭 세로 막대 파형, "듣는 중" 텍스트 제거', async ({ page }) => {
  ...
  expect(geometry.waveHeight).toBeCloseTo(100, 0);
...
test('B7 — 파형 막대 실제 렌더 + 주입 레벨에 진폭 반응(FB-D)', async ({ page }) => {
  ...
  const animation = await page.locator('[data-testid="voice-waveform"] span').nth(1).evaluate((el) => {
    const cs = getComputedStyle(el);
    return { name: cs.animationName, duration: cs.animationDuration, delay: cs.animationDelay };
  });
  expect(animation).toEqual({ name: 'inset-wave', duration: '0.94s', delay: '-0.073s' });
...
test('R1-4 — 일시정지: 파형 rAF 실중지(막대 정지), 재시작 시 재개', async ({ page }) => {
  ...
  expect(pausedBandBox!.height).toBeCloseTo(100, 0);
...
test('FB-I — 이상치 알람 카드가 흡수영역에 격리 — 파형/컨트롤 영역을 침범하지 않는다', async ({ page }) => {
  ...
  expect(bandBox!.height, '알람 중에도 파형 밴드는 100px 예약').toBeCloseTo(100, 0);
