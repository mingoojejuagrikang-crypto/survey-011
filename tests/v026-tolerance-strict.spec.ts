 ...
 *  T3 — 다이얼 aria/캡션이 "높을수록 엄격"을 명시(오해 재발 방지 가드).
 *
...
// ─── T3: 방향 문구 가드 — aria/캡션이 "높을수록 엄격" 명시 ──────────────────────
test('T3 — 허용범위 다이얼 aria-valuetext·캡션에 "높을수록 엄격" 명시', async ({ page }) => {
  await setupAndStart(page, 0.6);

  const dial = page.locator('[data-testid="dial-tolerance"]');
  await expect(dial).toBeVisible();
  const aria = await dial.locator('input[type="range"]').getAttribute('aria-valuetext');
  expect(aria).toContain('높을수록 엄격');
  await expect(dial.locator('text=높을수록 엄격')).toBeVisible(); // 눈에 보이는 캡션
});
...
test('T4 — 명령어 팝업: 마지막 항목(종료)·하단 닫기 버튼이 375px에서 스크롤 없이 전부 보임', async ({ page }) => {
  ...
  await page.locator('button:has-text("？ 명령어")').first().click();
