// 릴리스 일관성 가드: package.json 버전 ↔ README 버전 배지 ↔ CHANGELOG 맨 위 항목.
//
...
//
// v0.39.0 문서 정리에서 변경 내역이 README → CHANGELOG.md 로 분리됐다. 검사 대상도
// 따라 옮겼다: 배지는 여전히 README, 최신 항목은 CHANGELOG.
//
...
// (a) README "현재 버전: vX.Y.Z" 배지
const badge = readme.match(new RegExp(`현재 버전:\\s*v(${SEMVER})`));
// (b) CHANGELOG 맨 위 항목 "- **vX.Y.Z**"
const topEntry = changelog.match(new RegExp(`-\\s*\\*\\*v(${SEMVER})\\*\\*`));
...
if (topEntryVersion === null) {
  problems.push('CHANGELOG.md 에서 "- **vX.Y.Z**" 항목을 찾지 못했습니다.');
} else if (topEntryVersion !== pkgVersion) {
  problems.push(
    `CHANGELOG.md 맨 위 항목(v${topEntryVersion})이 package.json(v${pkgVersion})과 다릅니다.`,
  );
}
...
if (problems.length > 0) {
  ...
  console.error(
    '\n   → README.md 의 "현재 버전" 배지와 CHANGELOG.md 맨 위에 v' +
      pkgVersion +
      ' 항목을 추가한 뒤 다시 배포하세요.\n',
  );
  process.exit(1);
...
console.log(`✅ 릴리스 일관성 OK — package.json·README 배지·CHANGELOG 모두 v${pkgVersion}`);
