 ...
 *    mic 15.0% → 10.9% · alert 15.0% → 14.3% · pause 33.3% → 28.6% · check 6.1% → 6.3%
 *  **4종 전부 비율이 내려갔다** — 면적이 느는 방향은 하나도 남기지 않았다.
...
const GLYPHS: Record<DotGlyph, string[]> = {
  ...
  //   (획 폭 44.7 → 64.3px). 경고는 2~3m 밖 판독이 1순위다.
  //   🔴 **점만 4폭이다.** 격자 셀이 정사각형이 아니라서(열 피치 10.7px · 행 피치 12.1px)
  //   점을 획과 같은 6폭으로 두면 64.3×24.3px = **종횡비 2.65의 대시**가 된다(v0.45.0은
  //   3열×2행 = 44.7×34px = 1.31이었다). 4폭이면 42.8×24.3 = 1.76으로 점으로 읽힌다.
  //   ⚠️ 행을 늘려 비율을 맞추는 길은 막혀 있다 — 획 7 + 빈 1 + 점 2 = USABLE_ROWS 10이 꽉 찬다.
  alert: center(['######', '######', '######', '######', '######', '######', '######', '......', '####', '####']),
  // 일시정지: 막대 두 개. 폭 7/18(38.9%) → 12/25(48%)로 굵히면서도 비율은 33.3 → 28.6%로 내려간다.
  pause: center(['#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####', '#####..#####']),
  // 완료: 체크(짧은 하강 + 긴 상승). 15폭·획 2셀 — 1셀 획은 9px 도트에서 야외 판독이 무너진다.
  //   🔴 **v0.48.0 P1 — 꼭짓점을 뾰족하게 다시 그렸다(민구 확정 08-10).**
  //   제보: *"마지막행 입력후 도트 애니메이션의 체크 표시가 여전히 잘려있음"* → 민구가 지목한
  //   지점은 **두 선이 만나야 하는 하단 V 꼭짓점**이었다. 종전 비트맵은 바닥이 `#####` 5셀
  //   **수평**이라 두 획이 만나지 않고 평평하게 끊겼고, 위로도 r0·r1이 빈 채 8행에 눌려 있어
  //   긴 획 종단이 2셀 수평 **절단면**이었다(probe-new4 실측: 잉크가 격자 폭 54.9%·높이 47.9%).
  //   🔴 이건 「가림」이 아니다 — 필 겹침 0셀(W6 작동) · 조상 클립 0/22셀 · 캡처 왜곡 없음
  //   (민구 zip·네이티브·html2canvas 3자 ±1px). 남은 기전이 이 리터럴 하나였다.
  //
  //   지금 형상: **행 1~9 사용**(종전 2~8) · 양 끝은 2셀 → 1셀 **테이퍼**(절단면 소멸) ·
  //   r9의 2셀이 꼭짓점이고 r8에서 좌우로 갈라진다(r8 가운데 `.`가 V의 벌어짐이다 — 지우지 마라).
  //   🔑 **켜진 셀은 22개 그대로**다(6.3% — 발광 면적 §3-D 중립 유지). 획을 늘린 게 아니라
  //   같은 잉크를 격자에 **펴 놓은** 것이다: 세로 사용 7행 → 9행, 폭은 15 유지(잉크 열 6~18,
  //   중심 12 = 격자 중심). 그래서 이 변경에 정당 파손이 하나도 없다.
  //   ⚠️ 22셀은 우연이 아니라 **제약이다.** 첫 시안(r0까지 쓰고 짧은 획을 한 칸 늘린 것)은
  //   26셀 = 7.4%로 발광 계약을 건드렸다 — 그래서 r0를 비우고 짧은 획을 한 칸 줄여 되맞췄다.
  //   늘리고 싶으면 민구 결정(발광 계약 재해석)이 먼저다(`v0480-p1-check-vertex` ⓒ가 잡는다).
  //   ⚠️ 10행 계약은 그대로다 — r9까지가 `USABLE_ROWS`이고 하단 4행(필 자리)은 여전히 안 쓴다.
  //   오라클: `v0480-p1-check-vertex.spec.ts`([node] 꼭짓점 형상) · `v0440-c5-dots`([node] 10행).
  check: center([
    '...............',
    '.............#.',
    '............##.',
