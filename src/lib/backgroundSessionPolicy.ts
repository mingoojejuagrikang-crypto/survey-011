 ...
 *
 * ⚠️ iOS는 화면잠금·백그라운드에서 Web Audio·마이크를 OS 재량으로 중단시킬 수 있다.
 * 🔴 **근거 등급 정정(t9 조사 §2-2, 2026-08-08):** 종전 이 자리에 *"(연구 §3-B — 공식 확인)"*
 * 이라고 적혀 있었으나 **Apple 공식 답변은 존재하지 않는다.** 실제 근거는 *"사용자 제보 다수 ·
 * 반증 0"* 이다. 등급을 낮춰 적는 이유: 「공식 확인」은 재조사를 멈추게 하는 단어이고, 이 축은
 * 아직 **실기기로 확인해야 하는 가정**이다. 아래 임계·복귀 설계의 타당성은 이 등급과 무관하다
 * (10분 캡은 앱 자신의 계약이라 iOS 동작을 하나도 몰라도 성립한다 — t9 §3).
 * 이 게이트가 보장하는 것은 "앱이 스스로 죽이지 않는다"까지고, OS가 죽이는
