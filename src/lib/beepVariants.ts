 ...
 *  세그먼트 gain(0.04~0.055)에 곱한다. 주파수·클립경계 제약은 위 변형 정의에서 불변.
 *
 *  v0.44.0 F17(민구 확정 08-02: "2배로 진행") — 6 → 12. "안내음 2배"의 배선은 **이 마스터 배수
 *  한 곳**이다: 사용자 다이얼(0~1)의 모든 위치에서 일괄 2배가 되고(기본 0.5 → 6×, 최대 1.0 → 12×),
 *  변형 간 상대 밸런스(세그먼트 gain 대역)와 재생기 최종 클램프(beep.ts가 이 상수를 import)가
 *  자동으로 따라온다. 세그먼트 gain을 개별 2배 하면 대역 주석(0.04~0.055)이 깨지고 변형별로
 *  두 번 곱는 실수 표면이 생긴다. 클리핑 검산: 동시발음 최대 gain 합 0.07(pos-bell/neg-buzz)
 *  × 12 = 0.84 < 1.0 — 디지털 클리핑 없음. */
export const BEEP_VOLUME_MAX = 12;
