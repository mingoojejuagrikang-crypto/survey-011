...
export function useVoiceSession() {
  ...
  const handleFinal = useCallback(async (textArg: string, alts: string[], confidence: number) => {
    ...
    // 🔴 [ENV-12] E단계 E1 — 16 → 9. 명령 경로가 스테이지로 나가면서 그 경로 **전용** dep 8개가
    //   자연 소멸했다: `advance` `enterModifyMode` `relistenInContext` `goNextRow`
    //   `gotoAdjacentField` `gotoAdjacentRow` `clearAnomalyAlert`(잔류 구획 호출 0 — 실측) +
    //   `enterReviewWait`(분리 전부터 호출 0인 잔존 dep이었다). 대신 `runCommandStage` 하나가
    //   들어왔고 그건 `useCallback([])`+`depsRef`라 **영구 고정**이다.
    //   👉 fix49b #13이 지목했던 `gotoAdjacentField` 잠복 dep 경고는 이 이동으로 **소멸**한다 —
    //      그 호출이 이제 스테이지 안에 있고, 스테이지는 `depsRef.current`에서 매 호출 신선한
    //      값을 꺼내므로 「낡은 클로저로 dispatch」라는 드리프트 형태 자체가 성립하지 않는다.
    //   나머지 8개는 잔류 구획(D~H)이 실제로 부르는 것들이다 — E2~E4가 같은 방식으로 걷어낸다.
  }, [runCommandStage, proceedAfterCommit, rejectValue, say, persistSession, evaluateTrend, getAnomalyAlertData, archiveCellClip, armClipForCell]);
