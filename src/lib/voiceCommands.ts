...
export const VOICE_COMMANDS: CommandSpec[] = [
  ...
  //   🔴 v0.47.0 V-FIX4 — desc의 해제 안내를 **탭**으로 갱신했다(W7이 0.9초 홀드를 대체).
  //     도움말이 틀린 조작을 가르치면 사용자는 «켜지지 않는 화면»에 갇힌 것으로 받아들인다 —
  //     이 기능의 유일한 실패 모드가 그것이라 문구 1건도 계약이다.
  //     ⚠️ 진입 문구는 손대지 않는다: 음성 「화면」은 W7 이후에도 그대로 산다(홀드가 추가됐을 뿐).
  { id: 'screenOff',           word: '화면',             display: '화면 끄기',        desc: '화면을 꺼서 배터리를 아낍니다. 음성 입력은 계속됩니다 — 화면 가운데를 탭하면 다시 켜집니다' },
];
