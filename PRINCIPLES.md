...
- **확장은 「접두 불변 + 꼬리 추가」로만 한다** (v0.47.0 개정 — 이중 콜드 리뷰 U4).
  기존 `k=v` 쌍의 **순서·이름·값 형식은 바이트 불변**이고, 신규 필드는 **맨 뒤에만** 붙인다.
  사이에 끼워 넣는 것은 계약 위반이다. 소비자는 `$` 앵커가 아니라 **접두 매칭**으로 읽는다.
  - 🔴 **오라클은 「프로덕션이 실제로 방출하는 형상」을 재라.** 확장 필드가 항상 붙는 이벤트를
    ...
    테스트에만 존재하는 6필드 조합을 재고 있었다).
  - 접두를 **깨야 하는** 변경이면 필드를 고치지 말고 **새 이벤트 이름**을 써라.
